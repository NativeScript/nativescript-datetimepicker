...
export class DateTimePicker extends DateTimePickerBase {
  ...
  static pickDate(
    ...
    const pickDate = new Promise<Date>(resolve => {
      ...
      if (options.locale) {
        ...
        let preferredLocale = LocalizationUtils.createNativeLocale(
          options.locale
        );
  ...
  static pickTime(
    ...
    const pickTime = new Promise<Date>(resolve => {
      let originalLocale: java.util.Locale;
      if (options.locale) {
        originalLocale = options.context.getResources().getConfiguration()
          .locale;
        let preferredLocale = LocalizationUtils.createNativeLocale(
          options.locale
        );
        options.context
          .getResources()
          .getConfiguration().locale = preferredLocale;
      }
      ...
      const nativeDialog = DateTimePicker._createNativeDialog(
        ...
        (result: Date) => {
          if (originalLocale) {
            options.context
              .getResources()
              .getConfiguration().locale = originalLocale;
          }
          resolve(result);
        }
      );
  ...
  static _createNativeDatePicker(
    options: DatePickerOptions
  ...
  static _createNativeTimePicker(
    options: TimePickerOptions
  ...
  static _createNativeDialog(
    nativePicker: android.view.View,
    ...
    if (value) {
      ...
      dateTime =
        nativePicker instanceof android.widget.DatePicker
          ? getDateToday()
          : getDateNow();
    }
    ...
    const okButtonText = options.okButtonText
  ...
  static _showNativeDialog(
    nativePickerBuilder: android.app.AlertDialog.Builder,
    nativePicker: any,
    style: DateTimePickerStyle
  ) {
    let nativeDialog = nativePickerBuilder.show();
    if (style) {
      const buttonOkTextColor = style.buttonOkTextColor
        ? style.buttonOkTextColor
        : style.buttonsTextColor;
      const buttonOkBackgroundColor = style.buttonOkBackgroundColor
        ? style.buttonOkBackgroundColor
        : style.buttonsBackgroundColor;
      const buttonCancelTextColor = style.buttonCancelTextColor
        ? style.buttonCancelTextColor
        : style.buttonsTextColor;
      const buttonCancelBackgroundColor = style.buttonCancelBackgroundColor
        ? style.buttonCancelBackgroundColor
        : style.buttonsBackgroundColor;

      DateTimePicker._applyDialogColors(
        nativeDialog,
        style.titleTextColor,
        style.dialogBackgroundColor
      );
      DateTimePicker._applyDialogSpinnersColors(
        nativePicker,
        style.spinnersTextColor,
        style.spinnersBackgroundColor
      );
      DateTimePicker._applyDialogOkButtonColors(
        nativeDialog,
        buttonOkTextColor,
        buttonOkBackgroundColor
      );
      DateTimePicker._applyDialogCancelButtonColors(
        nativeDialog,
  ...
  private static _applyDialogColors(
    nativeDialog: android.app.AlertDialog,
    color: Color,
    ...
    if (backgroundColor) {
      nativeDialog
        ...
        .setBackgroundDrawable(
          new android.graphics.drawable.ColorDrawable(backgroundColor.android)
        );
    }
    if (color) {
      const label = DateTimePicker._findViewById<android.widget.TextView>(
        nativeDialog,
  ...
  private static _applyDialogSpinnersColors(
    ...
    if (color) {
      if (nativePicker instanceof android.widget.DatePicker) {
        const yearView = DateTimePicker._findViewById<
          android.widget.NumberPicker
        >(nativePicker, 'android:id/year');
        DateTimePicker._applyNumberPickerColor(yearView, color);
        const monthView = DateTimePicker._findViewById<
          android.widget.NumberPicker
        >(nativePicker, 'android:id/month');
        DateTimePicker._applyNumberPickerColor(monthView, color);
        const dayView = DateTimePicker._findViewById<
          android.widget.NumberPicker
        >(nativePicker, 'android:id/day');
        DateTimePicker._applyNumberPickerColor(dayView, color);
      } else if (nativePicker instanceof android.widget.TimePicker) {
        const hourView = DateTimePicker._findViewById<
          android.widget.NumberPicker
        >(nativePicker, 'android:id/hour');
        DateTimePicker._applyNumberPickerColor(hourView, color);
        const dividerView = DateTimePicker._findViewById<
          android.widget.TextView
        >(nativePicker, 'android:id/divider');
        DateTimePicker._applyTextViewColor(dividerView, color);
        const minuteView = DateTimePicker._findViewById<
          android.widget.NumberPicker
        >(nativePicker, 'android:id/minute');
        DateTimePicker._applyNumberPickerColor(minuteView, color);
        const amPmView = DateTimePicker._findViewById<
          android.widget.NumberPicker
        >(nativePicker, 'android:id/amPm');
  ...
  private static _applyDialogOkButtonColors(
    nativePicker: android.app.AlertDialog,
    color: Color,
    backgroundColor: Color
  ) {
    if (backgroundColor) {
      nativePicker
        ...
        .setBackgroundColor(backgroundColor.android);
    }
    if (color) {
      nativePicker
        .getButton(android.content.DialogInterface.BUTTON_POSITIVE)
        .setTextColor(color.android);
    }
  }
  ...
  private static _applyNumberPickerColor(
    ...
    const wheelPaint = DateTimePicker._findFieldByName(
      numberPicker,
      'mSelectorWheelPaint'
    );
    const selectionDividerDrawable = DateTimePicker._findFieldByName(
      numberPicker,
      'mSelectionDivider'
    );
    if (
      !wheelPaint ||
      !selectionDividerDrawable ||
  ...
  private static _findViewById<T extends android.view.View>(
    view: android.view.View | android.app.Dialog,
    id: string
  ): T {
    const searchId = view
      .getContext()

...
export class LocalizationUtils extends LocalizationUtilsBase {
  ...
  static createDefaultTimeFormat(context: any): string {
    ...
    const format = is24Hour
      ? LocalizationUtils.TIME_24H_FORMAT
      : LocalizationUtils.TIME_12H_FORMAT;
    return format;
  }

  static createNativeDateFormatter(
    ...
    let nativeDateFormat = new java.text.SimpleDateFormat(
      formatPattern,
      nativeLocale
    );
  ...
  static createNativeTimeFormatter(
    formatPattern: string,
    nativeLocale: any
  ): any {
    if (!formatPattern) {

import { Color, Device, IOSHelper, View } from '@nativescript/core';
import {
  DatePickerOptions,
  DateTimePickerBase,
...
export class DateTimePicker extends DateTimePickerBase {
  ...
  static _createNativeDialog(
    ...
    const alertSize = Math.min(
      alertController.view.frame.size.width,
      alertController.view.frame.size.height
    );
    const pickerViewWidth =
      UIDevice.currentDevice.userInterfaceIdiom === UIUserInterfaceIdiom.Pad
        ? DateTimePicker.PICKER_WIDTH_PAD
        : alertSize - DateTimePicker.PICKER_WIDTH_INSETS;
    ...
    if (style) {
      spinnersBackgroundColor = style.spinnersBackgroundColor
    ...
    const messageLabel = DateTimePicker._findLabelWithText(
      alertController.view,
      DateTimePicker.PICKER_DEFAULT_MESSAGE
    );
    const messageLabelContainer = DateTimePicker._getLabelContainer(
      messageLabel
    );
    ...
    pickerContainer.topAnchor.constraintEqualToAnchorConstant(
      ...
      pickerContainerFrameTop
    ).active = true;
    pickerContainer.leftAnchor.constraintEqualToAnchor(
      alertController.view.leftAnchor
    ).active = true;
    pickerContainer.rightAnchor.constraintEqualToAnchor(
      alertController.view.rightAnchor
    ).active = true;
    pickerContainer.bottomAnchor.constraintEqualToAnchor(
      alertController.view.bottomAnchor
    ).active = true;

    pickerView.leftAnchor.constraintLessThanOrEqualToAnchorConstant(
      pickerContainer.leftAnchor,
      DateTimePicker.PICKER_WIDTH_INSETS
    ).active = true;
    pickerView.rightAnchor.constraintLessThanOrEqualToAnchorConstant(
    ...
    const cancelActionStyle =
      style && style.buttonCancelBackgroundColor
    ...
    let okAction = UIAlertAction.actionWithTitleStyleHandler(
      okButtonText,
      UIAlertActionStyle.Default,
      () => {
    ...
    if (style) {
      ...
      DateTimePicker._applyDialogButtonTextColor(
        cancelAction,
        buttonCancelTextColor
      );
      DateTimePicker._applyDialogTitleTextColor(
        alertController,
  ...
  static _showNativeDialog(
    nativeDialog: UIAlertController,
    nativePicker: UIDatePicker,
    style: DateTimePickerStyle
  ) {
    ...
    if (currentPage) {
      ...
      if (currentPage.modal) {
        ...
        if (view.ios instanceof UIViewController) {
          ...
          const parentWithController = IOSHelper.getParentWithViewController(
            view
          );
      ...
      if (viewController) {
        if (nativeDialog.popoverPresentationController) {
          nativeDialog.popoverPresentationController.sourceView =
            viewController.view;
          nativeDialog.popoverPresentationController.sourceRect = CGRectMake(
  ...
  private static _applyDialogTitleTextColor(
    ...
    if (color) {
      if (nativeDialog.title) {
        let title = NSAttributedString.alloc().initWithStringAttributes(
          nativeDialog.title,
          <any>{ [NSForegroundColorAttributeName]: color.ios }
        );
  ...
  private static _clearVibrancyEffects(uiView: UIView) {
    if (
      uiView instanceof UIVisualEffectView &&
      uiView.effect instanceof UIVibrancyEffect
    ) {

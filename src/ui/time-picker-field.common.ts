import { CSSType, EventData, Property } from '@nativescript/core';
import { DateTimePicker, DateTimePickerStyle } from '../datetimepicker';
import { dateComparer, getDateNow } from '../utils/date-utils';
...
export class TimePickerFieldBase
  extends PickerFieldBase
  implements TimePickerFieldDefinition {
  public time: Date;
  ...
  private _initRegionalSettings() {
    ...
    this._nativeTimeFormatter = LocalizationUtils.createNativeTimeFormatter(
      this.timeFormat,
      this._nativeLocale
    );
  }
...
export function timeValueConverter(timeString: string) {
  ...
  timeRemainder = timeRemainder.startsWith(':')
    ? timeRemainder.substring(1)
    : timeRemainder;
